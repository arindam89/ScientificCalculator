...
export default function MemoryFunctions({ onMemoryOperation }: MemoryFunctionsProps) {
  ...
  return (
    <div className="grid grid-cols-5 gap-2 p-3 pt-3 pb-0 bg-gray-50">
      {memoryButtons.map((button) => (
        <button
          key={button.label}
          className="calculator-btn bg-gray-200 text-slate-900 hover:bg-gray-300 rounded-lg p-2 text-sm font-medium transition-colors"
